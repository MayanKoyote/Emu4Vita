			   ...
			   const FT_BitmapGlyph bitmap_glyph, int glyph_size)
{
	...
	tex_width = vita2d_texture_get_width(tex);
									...
									const char *text)
{
	...
	for (i = 0; text[i];) {
		...
		if (!texture_atlas_get(font->atlas, glyph_index, &tex, &rect, &data)) {
			...
			if (!atlas_add_glyph(font->atlas, glyph_index, (FT_BitmapGlyph)glyph, size))
				continue;
